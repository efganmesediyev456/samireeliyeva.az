...
class ImportantLinksDataTable extends DataTable
{
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($item) {
                return view('backend.pages.important_links.actions', compact('item'), [
                    'editRoute' => route('admin.important_links.edit', $item->id),
                    'deleteRoute' => route('admin.important_links.destroy', $item->id),
                ]);
            })
            ->editColumn('image', function ($item) {
                if ($item->image) {
                    return '<img src="' . asset("storage/{$item->image}") . '" width="80">';
                }
                return 'N/A';
            })
    ...
    public function query(ImportantLink $model): QueryBuilder
    {
    ...
    {
        return $this->builder()
            ->setTableId('important-links-table')
                        ...
                        if (data.length) {
                            $.ajax({
                                ...
                                data: {
                                    ...
                                    items: data,
                                    model:'" . addslashes(ImportantLink::class) . "'
                                },
