...
class TextbooksDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($item) {
                ...
                $edit = '<a href="'.route('admin.textbooks.edit', $item->id).'" class="btn btn-sm fs-5"><i class="fas fa-edit"></i></a>';
                $delete = '<form action="' . route('admin.textbooks.destroy', $item->id) . '" method="POST" class="delete-form">
                                    <input type="hidden" name="_token" value="' . csrf_token() . '">
                                    <input type="hidden" name="_method" value="DELETE">
                                    <button type="submit" class="btn btn-sm fs-5 text-danger delete-btn"><i class="fas fa-trash"></i></button>
                            </form>';

                // Düzgün marşrutu istifadə edin
                $addAttributes = '<a href="'.route('admin.textbooks.attributes.index', $item->id).'" class="btn btn-sm"><i class="fas fa-plus"></i></a>';

                $html .= $edit . $delete . $addAttributes;
                $html .= '</div>';
                return $html;
            })
    ...
    public function query(Textbook $model): QueryBuilder
    {
                        ...
                        if (data.length) {
                            $.ajax({
                                ...
                                data: {
                                    ...
                                    items: data,
                                    model:'".addslashes(Textbook::class)."'
                                },
