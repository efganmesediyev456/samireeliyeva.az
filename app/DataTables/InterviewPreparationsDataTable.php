...
class InterviewPreparationsDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($item) {
                return view('backend.pages.interview-preparations.actions', compact('item'), [
                    'editRoute' => route('admin.subcategories.interview-preparations.edit', [$this->subcategory->id, $item->id]),
                    'deleteRoute' => route('admin.subcategories.interview-preparations.destroy', [$this->subcategory->id, $item->id]),
                ]);
            })
            ...
            ->editColumn('video_url', function ($item) {
                ...
                return $item->video_url ? '<a href="'.$item->video_url.'" target="_blank">Video Linki</a>' : 'N/A';
            })
            ...
            ->rawColumns(['action', 'thumbnail', 'video_url', 'status'])
            ->setRowId('id');
    }

    public function query(InterviewPreparation $model): QueryBuilder
    {
    ...
    {
        return $this->builder()
            ->setTableId('interview-preparations-table')
                        ...
                        if (data.length) {
                            $.ajax({
                                ...
                                data: {
                                    ...
                                    items: data,
                                    model:'".addslashes(InterviewPreparation::class)."'
                                },
    ...
    {
        return 'InterviewPreparations_' . date('YmdHis');
    }
}