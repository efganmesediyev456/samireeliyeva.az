    ...
    <div class="card">
        <div class="card-header">
            <div class="d-flex justify-content-between align-items-center">
                <h4>İmtahan Banneri</h4>
            </div>
        </div>
        <div class="card-body">
            <form action="{{route('admin.exam_banner.update', $item->id)}}" method="POST" id="saveForm" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="tab-content mt-3" id="languageTabsContent">
                    ...
                    <div class="tab-pane fade show @if($loop->iteration==1) active @endif" id="{{$language->code}}"
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-12">
                                <div class="mb-3">
                                    <label for="exam_online_title{{$language->code}}" class="form-label">Onlayn imtahan Başlıq {{$language->code}}</label>
                                    <input type="text" class="form-control" name="exam_online_title[{{$language->code}}]"
                                        id="exam_online_title{{$language->code}}"
                                        placeholder="Başlıq daxil edin"
                                        value="{{$item->getTranslation($language->code)?->exam_online_title}}">
                                </div>
                            </div>
                ...
                <div class="row mt-3">
                    <div class="col-md-6">
                     ...
                     <div class="col-md-6">
                        <div class="mb-3">
                            <label for="image" class="form-label">Onlayn imtahan Şəkil</label>
                            <input type="file" name="exam_online_image" class="form-control">
                            @if($item->exam_online_image)
                                <img width="300" src="/storage/{{$item->exam_online_image}}" alt="" class="mt-2">
                            @endif
                        </div>
                    </div>
                </div>
