...
$("body").on("click",'.edit', function (e){
    ...
    $.ajax({
        ...
        success:function (json){
            ...
            translate("#description-edit","description", json, true);
            setTimeout(function (){
                $("#order-edit").val(json.order);
            }, 500)
            if(json.status)
                $("#status-edit").prop('checked', true)

            $("#image-edit").siblings('img').remove();
            $("#image-edit").after('<img width="300px" class="my-3" src="/storage/categories/'+json.image+'">')

            $("#parent_id-edit").val(json.parent_id).trigger('change')
